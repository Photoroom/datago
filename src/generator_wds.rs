    ...
    if config.url.contains("{") {
        ...
        if let Some(items) = response_json.get("items") {
            for item in items.as_array().unwrap() {
                match item.get("mediaLink") {
                    Some(media_link) => {
                        if pages_tx.send(media_link.clone()).is_err() {
                            debug!("Failed to send item");
                            break;
                        }
                        debug!("Pushed new item: {}", media_link);
                        count += 1;
                    }
                    None => {
                        warn!("No media link found for item");
                    }
                }
            }
        } else {
...
fn group_samples(samples: TarballContent) -> Result<HashMap<i32, TarballContent>, Box<dyn Error>> {
        ...
        match bytes_from_url(&shared_client, response_json.as_str().unwrap()).await {
            Some(tarball) => {
                ...
                if let Ok(contents) = untar_bytes_in_memory(&tarball).await {
                    let samples = group_samples(contents).unwrap();

                    for (_, sample) in samples.into_iter() {
...
fn dispatch_shards(
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .unwrap()
        .block_on(async {
            _ = async_pull_and_dispatch_tarballs(shared_client.clone(), pages_rx, samples_meta_tx)
                .await;
        });
}
...
pub fn orchestrate(client: &DatagoClient) -> DatagoEngine {
    ...
    let feeder = Some(thread::spawn(move || {
        dispatch_shards(shared_client, pages_rx_pinger, samples_meta_tx);
    }));
...
mod tests {
    ...
    fn test_webdataset_dispatch() {
        ...
        let dispatcher = thread::spawn(move || {
            dispatch_shards(http_client, pages_rx_client, samples_meta_tx_client);
        });
